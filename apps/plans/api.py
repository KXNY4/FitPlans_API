...
def list_categories(request):
    """Список категорий."""
    categories = Category.objects.annotate(plans_count=Count("plans", filter=Q(plans__status="PUBLISHED")))
...
def create_plan(request, data: PlanCreateIn):
    ...
    plan = PlanService.create_plan(trainer=request.auth.trainer_profile, **data.model_dump())
...
def update_plan(request, plan_id: UUID, data: PlanUpdateIn):
    """Обновить план."""
    plan = PlanService.update_plan(plan_id=plan_id, user=request.auth, **data.model_dump(exclude_unset=True))
